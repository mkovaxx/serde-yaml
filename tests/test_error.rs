...
use serde_yaml_ok::value::{Tag, TaggedValue};
use serde_yaml_ok::{Deserializer, Value};
#[cfg(not(miri))]
...
{
    let result = serde_yaml_ok::from_str::<T>(yaml);
...
fn test_serialize_nested_enum() {
    ...
    let error = serde_yaml_ok::to_string(&e).unwrap_err();
